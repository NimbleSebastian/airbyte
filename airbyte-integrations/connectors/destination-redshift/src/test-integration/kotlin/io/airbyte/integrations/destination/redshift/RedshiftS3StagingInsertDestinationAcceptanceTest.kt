...
/**
 * Integration test testing [RedshiftStagingS3Destination]. The default Redshift integration
...
class RedshiftS3StagingInsertDestinationAcceptanceTest : JdbcDestinationAcceptanceTest() {
  ...
  private fun retrieveRecordsFromTable(tableName: String, schemaName: String?): List<JsonNode> {
    return database!!.query<List<JsonNode>> { ctx: DSLContext ->
      ctx
        .fetch(String.format("SELECT * FROM %s.%s ORDER BY %s ASC;", schemaName, tableName, COLUMN_NAME_EMITTED_AT))
        .stream()
  ...
  override fun setup(testEnv: TestDestinationEnv, TEST_SCHEMAS: HashSet<String>) {
    ...
    val createUser = String.format(
      "create user %s with password '%s' SESSION TIMEOUT 60;",
      USER_WITHOUT_CREDS, baseConfig!!["password"].asText()
    )
  ...
  private fun removeOldNamespaces() {
    ...
    try {
      schemas =
        database!!.query { ctx: DSLContext -> ctx.fetch("SELECT schema_name FROM information_schema.schemata;") }!!
          .stream()
    ...
    for (schema in schemas) {
      if (isOlderThan2Days(schema)) {
        try {
          database!!.query { ctx: DSLContext -> ctx.execute(String.format("DROP SCHEMA IF EXISTS %s CASCADE", schema)) }
          schemasDeletedCount++
        } catch (e: SQLException) {
  ...
  override fun tearDown(testEnv: TestDestinationEnv) {
    ...
    database!!.query { ctx: DSLContext -> ctx.execute(String.format("drop user if exists %s;", USER_WITHOUT_CREDS)) }
  ...
  companion object {
    private val LOGGER: Logger = LoggerFactory.getLogger(
      RedshiftS3StagingInsertDestinationAcceptanceTest::class.java
    )
  }
}
