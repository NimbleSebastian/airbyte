...
class RedshiftSqlGeneratorTest {
  ...
  @BeforeEach
  fun setup() {
    streamId = StreamId("test_schema", "users_final", "test_schema", "users_raw", "test_schema", "users_final")
    ...
    columns[redshiftSqlGenerator.buildColumnId("struct")] =
      Struct(LinkedHashMap())
    columns[redshiftSqlGenerator.buildColumnId("array")] =
    ...
    columns[redshiftSqlGenerator.buildColumnId("integer")] =
      AirbyteProtocolType.INTEGER
    columns[redshiftSqlGenerator.buildColumnId("boolean")] =
  ...
  fun testTypingAndDeduping() {
    ...
    val expectedSqlLines = Arrays.stream(expectedSql.split("\n".toRegex()).dropLastWhile { it.isEmpty() }
      .toTypedArray()).map { obj: String -> obj.trim { it <= ' ' } }.toList()
    val generatedSqlLines = generatedSql.asSqlStrings("BEGIN", "COMMIT").stream()
  ...
  fun test2000ColumnSql() {
    ...
    for (i in 0..1999) {
      val columnName = RANDOM
        .ints('a'.code, 'z'.code + 1)
        .limit(15)
        .collect(
          { StringBuilder() },
    ...
    val generatedSql = redshiftSqlGenerator.updateTable(
      StreamConfig(
        streamId!!,
        DestinationSyncMode.APPEND_DEDUP,
        primaryKey,
        Optional.of(cursor),
        columns, 0, 0, 0
      ), "unittest", Optional.of(Instant.parse("2023-02-15T18:35:24.00Z")), false
    )
