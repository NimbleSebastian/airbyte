...
class RedshiftUtilTest {
  ...
  fun testFindS3OptionsWhenConfigHasUploadingMethod() {
    ...
    val uploadingMethod = Mockito.mock(
      JsonNode::class.java
    )
  ...
  fun testAllS3RequiredAreNotNullOrEmptyThenReturnFalse() {
    ...
    Mockito.`when`(jsonNode["s3_bucket_name"]).thenReturn(
      Mockito.mock(
        JsonNode::class.java
      )
    )
    Mockito.`when`(jsonNode["s3_bucket_name"].asText()).thenReturn("test")
    Mockito.`when`(jsonNode["s3_bucket_region"]).thenReturn(
      Mockito.mock(
        JsonNode::class.java
      )
    )
    Mockito.`when`(jsonNode["s3_bucket_region"].asText()).thenReturn("test")
    Mockito.`when`(jsonNode["access_key_id"]).thenReturn(
      Mockito.mock(
        JsonNode::class.java
      )
    )
    Mockito.`when`(jsonNode["access_key_id"].asText()).thenReturn("test")
    Mockito.`when`(jsonNode["secret_access_key"]).thenReturn(
      Mockito.mock(
        JsonNode::class.java
      )
    )
