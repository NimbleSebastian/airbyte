...
class RedshiftSqlGeneratorIntegrationTest : JdbcSqlGeneratorIntegrationTest<RedshiftState>() {
  ...
  /**
   * Redshift's JDBC driver doesn't map certain data types onto [java.sql.JDBCType] usefully.
  ...
  class RedshiftSourceOperations : JdbcSourceOperations() {
    ...
    override fun copyToJsonField(resultSet: ResultSet, colIndex: Int, json: ObjectNode) {
      ...
      when (columnTypeName) {
        "super" -> json.set<JsonNode>(columnName, deserializeExact(resultSet.getString(colIndex)))
    ...
    override fun putTimeWithTimezone(
      ...
      val offsetTime = resultSet.getTimestamp(index).toInstant().atOffset(ZoneOffset.UTC).toOffsetTime()
    ...
    @Throws(SQLException::class)
    override fun putTimestampWithTimezone(node: ObjectNode, columnName: String?, resultSet: ResultSet, index: Int) {
    ...
    @Throws(SQLException::class)
    override fun putTimestamp(node: ObjectNode, columnName: String?, resultSet: ResultSet, index: Int) {
      try {
        node.put(columnName, convertToTimestamp(getObject(resultSet, index, LocalDateTime::class.java)))
      } catch (e: Exception) {
  ...
  override val sqlGenerator: JdbcSqlGenerator
    get() = object : RedshiftSqlGenerator(RedshiftSQLNameTransformer(), false) {
  ...
  override fun toJsonValue(valueAsString: String?): Field<*> {
    return DSL.function("JSON_PARSE", String::class.java, DSL.`val`(escapeStringLiteral(valueAsString)))
  }
  ...
  fun testCascadeDropEnabled() {
    ...
    Companion.database!!.execute(
      DSL.createView(DSL.quotedName(incrementalAppendStream.id.finalNamespace, "example_view"))
        .`as`(
          DSL.select()
            .from(DSL.quotedName(incrementalAppendStream.id.finalNamespace, incrementalAppendStream.id.finalName))
        )
        .getSQL(ParamType.INLINED)
    )
    // Create a "soft reset" table
    destinationHandler.execute(generator.createTable(incrementalDedupStream, "_soft_reset", false))

    // Overwriting the first table with the second table should succeed.
    Assertions.assertDoesNotThrow {
      destinationHandler.execute(
        generator.overwriteFinalTable(
  ...
  fun testCascadeDropDisabled() {
    ...
    val t: Throwable = Assertions.assertThrowsExactly(
      ConfigErrorException::class.java
    ) { destinationHandler.execute(generator.overwriteFinalTable(incrementalDedupStream.id, "_soft_reset")) }
    Assertions.assertTrue(t.message == "Failed to drop table without the CASCADE option. Consider changing the drop_cascade configuration parameter")
  }
  ...
  companion object {
    ...
    fun setupJdbcDatasource() {
      ...
      database = insertDestination.getDatabase(
        dataSource, RedshiftSourceOperations()
      )
    }
