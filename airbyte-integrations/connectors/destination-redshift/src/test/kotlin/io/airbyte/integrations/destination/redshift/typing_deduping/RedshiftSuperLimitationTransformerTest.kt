...
class RedshiftSuperLimitationTransformerTest {
  ...
  fun setup() {
    ...
    IntStream.range(3, 1025).forEach { i: Int ->
      columns[redshiftSqlGenerator.buildColumnId(
        "column$i"
      )] = AirbyteProtocolType.STRING
    }

    val streamId = StreamId("test_schema", "users_final", "test_schema", "users_raw", "test_schema", "users_final")
  ...
  fun testVarcharNulling() {
    ...
    val jacksonDeserializationSize = serialize(jsonNode).toByteArray(StandardCharsets.UTF_8).size
    ...
    val jacksonDeserializeSizeAfterTransform = serialize(jsonNode).toByteArray(StandardCharsets.UTF_8).size
  ...
  fun testRedshiftSuperLimit_ShouldRemovePartialRecord() {
    ...
    val upstreamMeta = AirbyteRecordMessageMeta()
      .withChanges(
        List.of(
    ...
    Assertions.assertTrue(
      serialize(transformed.first!!)
        .toByteArray(StandardCharsets.UTF_8).size < RedshiftSuperLimitationTransformer.REDSHIFT_SUPER_MAX_BYTE_SIZE
    )
    ...
    Assertions.assertEquals(AirbyteRecordMessageMetaChange.Change.NULLED, transformed.second!!.changes.first.change)
    Assertions.assertEquals(
  ...
  fun testRedshiftSuperLimit_ShouldFailOnPKMissing() {
    ...
    val ex: Exception = Assertions.assertThrows(
